...
**Histórico de versões:** ver `CHANGELOG.md` (formato Keep a Changelog) — fonte canônica do "o que mudou" por versão, linguagem voltada a release. O texto abaixo entra em detalhe técnico (arquivos, causa raiz, decisões de implementação); ao entregar uma nova versão, atualizar os dois.

**Última versão entregue:** v1.1.6 (2026-09-13) — feature de "mês contábil" configurável (dia de início do mês, 1–31) para relatórios e card "Este mês" das Finanças, substituindo o mês calendário fixo (dia 01 ao último dia). Tabela `settings` (`Setting::get/set`) + `PeriodService` (`currentMonthPeriod()`/`periodContaining()`/`nextPeriod()`, sempre em `business_timezone`, com fallback para o último dia válido em meses curtos) + endpoints `GET /api/period/current` (qualquer autenticado) e `GET`/`PUT /api/settings` (`role:adm`, validação 1–31). `FinanceController::summary()`/`seriesByMonth()` migrados para `PeriodService`, corrigindo de quebra um bug de timezone (`Carbon::now()` sem `business_timezone` — card "Hoje" mostrava hora UTC em vez de horário de Brasília). Frontend: `RelatoriosPage.tsx` não calcula mais o período localmente (`getMonthRange()` removido, duplicava lógica com o backend) — consome `/api/period/current`; nova tela `/configuracoes` (ADM-only) para editar `dia_inicio_mes`. Com migration nova (`2026_09_13_000001_create_settings_table`). Bundle `index-D3ppEq7Z.js`. Pacote de update em `D:\ProjDev\Atualizações\RenovatPneus\RenovatPneus-Update-v1.1.6\`. **Limitação conhecida (não bloqueante):** com `dia_inicio_mes != 1`, o gráfico anual de Finanças pode exibir um bucket parcial do ano anterior (ver comentário em `FinanceController::seriesByMonth()`) — decisão de UX pendente.
Anterior: v1.0.6 (2026-08-06) — filtros de Estoque (busca/marca/status) movidos para server-side (antes filtravam só os 15 itens da página atual, causando página vazia após filtro); novo endpoint `GET /api/products/brands` (lista completa de marcas, não só da página); `Product.brand`/`size` tipados como `string | null` no frontend + guardas `|| '—'` em todo lugar que renderiza esses campos (Estoque, Entrada, Saida, Scanner). Sem migration — só `ProductController.php` + `routes/api.php` + frontend novo (bundle `index-_uM8Q3U9.js`). Pacote arquivado em `D:\ProjDev\Atualizações\RenovatPneus\RenovatPneus-Update-v1.0.6\`. **`update.bat` deste pacote só para/reinicia o serviço `RenovatPneusAPI`** (não mexe no `RenovatPneusSSL`/stunnel, já que nada relacionado a TLS mudou) — diferente do padrão v1.0.4+ que parava ambos.
Anterior: v1.0.5 (data não registrada no momento da entrega — confirmado retroativamente pelo bundle `index-CAqfLqd_.js` visto em produção) — fix erro 500 ao cadastrar usuário sem e-mail (`email` passou a aceitar `null` — migration `2026_08_03_000001_make_email_nullable_on_users_table`).
Anterior: v1.0.4 (2026-06-23) — (1) fix Chrome Tradutor: `translate="no"` + `lang="pt-BR"` no `<html>` e `<meta name="google" content="notranslate">` no `index.html` — o tradutor automático substituía nós de texto fora do React causando `NotFoundError: removeChild`; (2) fix busca no Estoque: guard `?? ''` em `name`/`size` e `.filter(Boolean)` nas marcas para campos `null` no banco (bundle `index-1T2Qwnhp.js`).
...
- **Rede com DHCP (sem IP fixo):** `EnsureFrontendRequestsAreStateful::fromFrontend()` (vendor `laravel/sanctum`) compara o `Origin`/`Referer` contra cada entrada de `SANCTUM_STATEFUL_DOMAINS` via `Str::is()`, que aceita wildcard `*`. Em vez de IP fixo, usar padrão de subnet: `192.168.0.*:8000,192.168.0.*:8443` — sobrevive à troca de IP pelo DHCP dentro da mesma subnet. Só quebra se o roteador mudar de faixa (raro)
- **Não assumir que a subnet de produção é igual à do PC de desenvolvimento** — já causou diagnóstico errado uma vez (dev em `192.168.1.x`, produção em `192.168.0.x`). Confirmar sempre na própria máquina de produção: `Get-NetIPAddress -AddressFamily IPv4 | Where-Object { $_.IPAddress -match '^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.)' }`
- **Edição manual do `.env` em produção não basta** — se `config:cache` já rodou (padrão pós-instalação), o Laravel lê o array compilado em `bootstrap/cache/config.php` e ignora mudanças no `.env` até rodar `artisan config:clear` + `config:cache` de novo. Restart do serviço NSSM sozinho **não** regenera esse cache
- **Sintoma característico desse grupo de problemas:** login retorna 200 normalmente (o controller de login não depende do middleware stateful), mas todo `GET` autenticado seguinte (`/api/me`, `/api/dashboard`, `/api/sales`, etc.) cai em 401 — a falha é silenciosa e só aparece nas chamadas subsequentes
...
| Update v1.0.5 — fix erro 500 ao cadastrar usuário sem e-mail | ✅ Entregue (confirmado retroativamente) |
| Update v1.0.6 — filtros de Estoque server-side + endpoint de marcas + null-safety brand/size | ✅ Entregue 2026-08-06 |
| Update v1.1.6 — mês contábil configurável (dia_inicio_mes) + fix timezone Finanças | ✅ Entregue 2026-09-13 |
